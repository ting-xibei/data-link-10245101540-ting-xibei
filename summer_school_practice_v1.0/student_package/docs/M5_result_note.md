...
- 字段缺失、帧验证失败、来源真实性三者的区别：位置字段缺失触发 R1，但不等于帧无效；仅 `message_valid=false` 或上游明确携带 `validation_errors` 时，选做规则才生成 `FRAME_VALIDATION_ERROR`；`message_valid` 只表示 TeachingLink 帧结构和接收校验，不证明来源真实或飞行状态安全。
